function createGeneratedItem(THREE) {
    ...
    const components = [{
        ...
        "position": { "x": 0, "y": 6, "z": 1.8 },
        "rotation": { "x": 0, "y": 0, "z": 0 },
        "color": "#4a6e45"
    }, {
        "name": "Hood",
        "shape": "box",
        "dimensions": { "x": 4.5, "y": 4.5, "z": 4.5 },
    ...
    const allComponents = components.concat(swordComponents);
    ...
    allComponents.forEach(c => {
        ...
        switch (c.shape) {
            case 'cylinder':
                ...
                break;
            case 'box':
            default: // Handles "prism" as a box
                geometry = new THREE.BoxGeometry(dims.x * scale, dims.y * scale, dims.z * scale);
                break;
        }

        const color = new THREE.Color(c.color).getHex();
